...
-- Composite unique: one value per (branch, key) pair
CREATE UNIQUE INDEX IF NOT EXISTS idx_settings_branch_key
  ON settings(branch_id, key) WHERE branch_id IS NOT NULL;

-- Partial index: global keys (branch_id IS NULL) must also be unique
