...
export const listQuestions = asyncHandler(async (req: Request, res: Response) => {
  const { q, tag, type, category, treatment_tag, body_region, is_active = "true" } = req.query as Record<string, string>;
  ...
  if (treatment_tag) { conditions.push(`$${i++} = ANY(treatment_tags)`);     vals.push(treatment_tag); }
  if (body_region)   { conditions.push(`$${i++} = ANY(body_regions)`);       vals.push(body_region); }
  const result = await query(
    `SELECT * FROM questions WHERE ${conditions.join(" AND ")} ORDER BY category, text`,
    vals
  );
...
export const updateQuestion = asyncHandler(async (req: Request, res: Response) => {
  const allowed = ["text","answer_type","options","scale_min","scale_max","tags","category","treatment_tags","body_regions","is_active"];
