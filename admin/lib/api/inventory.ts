...
export const inventoryApi = {
    ...
    createItem: (data: { name: string; category: InventoryCategory; unit: string; current_stock: number; min_stock: number; notes?: string }) =>
        request<InventoryItem>("/inventory/items", { method: "POST", body: JSON.stringify(data) }),
    updateItem: (id: string, data: Partial<{ name: string; category: InventoryCategory; unit: string; min_stock: number; notes: string; is_active: boolean }>) =>
        request<InventoryItem>(`/inventory/items/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
    deactivateItem: (id: string) =>
        request<{ ok: boolean }>(`/inventory/items/${id}`, { method: "DELETE" }),
