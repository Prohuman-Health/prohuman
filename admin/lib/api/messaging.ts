...
export const messagingApi = {
    ...
    createChannel: (data: { name: string; description?: string; member_ids?: string[] }) =>
        request<StaffChannel>("/messages/channels", { method: "POST", body: JSON.stringify(data) }),
    ...
    addMember: (channelId: string, staff_id: string) =>
        request<{ ok: boolean }>(`/messages/channels/${channelId}/members`, { method: "POST", body: JSON.stringify({ staff_id }) }),
};
