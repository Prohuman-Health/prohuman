...
function SessionsTab({ sessions, onSchedule }: { sessions: PatientSession[]; onSchedule: () => void }) {
    const [openForms, setOpenForms] = useState<Record<string, boolean>>({});

    function toggleForm(id: string) { setOpenForms((prev: Record<string, boolean>) => ({ ...prev, [id]: !prev[id] })); }
...
function InvoicesTab({ patientId, invoices: initialInvoices, loading: externalLoading }: {
    patientId: string;
    invoices: PatientInvoice[];
    loading: boolean;
}) {
    const [invoices, setInvoices] = useState<PatientInvoice[]>(initialInvoices);
    const [editId, setEditId] = useState<string | null>(null);
    const [editStatus, setEditStatus] = useState<"pending" | "paid" | "waived">("pending");
    const [editNotes, setEditNotes] = useState("");
    const [saving, setSaving] = useState(false);

    // Sync when parent updates initial data (e.g. after lazy load)
    ...
    async function saveEdit() {
        ...
        try {
            ...
            setEditId(null);
            const updated = await patientsApi.invoices(patientId);
            setInvoices(updated);
    ...
    if (externalLoading) return <div className="space-y-3 p-1">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20" />)}</div>;
...
export default function PatientDetailPage() {
    ...
    // Load patient + sessions + timeline together
    const loadAll = useCallback(async () => {
        setLoading(true); setError(null);
        try {
            const [p, sess, tl] = await Promise.all([
                patientsApi.get(patientId),
                patientsApi.sessions(patientId),
                patientsApi.timeline(patientId),
            ]);
            setPatient(p);
            setSessions(sess as PatientSession[]);
    ...
    useEffect(() => { loadAll(); loadLabels(); }, [loadAll, loadLabels]);

    // Lazy-load invoices once when invoices tab is first opened
    useEffect(() => {
        if (tab === "invoices" && !invoicesLoaded && !invoicesLoading) {
            setInvoicesLoading(true);
            patientsApi.invoices(patientId)
                .then(d => { setInvoices(d); setInvoicesLoaded(true); })
                .catch(() => {})
                .finally(() => setInvoicesLoading(false));
        }
    }, [tab, invoicesLoaded, invoicesLoading, patientId]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5 items-start">
                ...
                <div className="bg-white rounded-2xl border border-border/50 flex flex-col overflow-hidden min-h-[500px]">
                    ...
                    <div className="flex-1 p-5 overflow-y-auto">
                        {tab === "sessions" && <SessionsTab sessions={sessions} onSchedule={() => setScheduleOpen(true)} />}
                        {tab === "invoices" && <InvoicesTab patientId={patient.id} invoices={invoices} loading={invoicesLoading} />}
