...
import {
    ...
    BookOpen, AlertTriangle, ClipboardCheck, Dumbbell, RefreshCw,
} from "lucide-react";
...
import { NewAlgorithmModal, EditAlgorithmModal, DeleteConfirmModal } from "@/components/modals/algorithm-modals";
import { ApplyAlgorithmModal } from "@/components/modals/apply-algorithm-modal";
...
export default function AlgorithmsPage() {
    ...
    const [deleting, setDeleting] = useState<Algorithm | null>(null);
    const [applying, setApplying] = useState<Algorithm | null>(null);
    ...
    return (
        <>
            <div className="flex flex-col gap-4 p-4 md:p-5">
                ...
                <div className="space-y-2">
                    {loading ? (
                        ...
                        const exercises = (a as Algorithm & { exercises?: unknown[] }).exercises ?? [];
                        const exerciseNames = exercises.map((e: unknown) =>
                            (e as { exercise_name?: string }).exercise_name ?? String(e)
                        ).filter(Boolean);
                                ...
                                <div className="flex items-center gap-4 px-4 md:px-5 py-3.5 cursor-pointer hover:bg-muted/20 transition-colors"
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        ...
                                        <button
                                            ...
                                            onClick={ev => { ev.stopPropagation(); setDeleting(a); }}>
            ...
            {/* Modals */}
            <NewAlgorithmModal open={showAdd} onClose={() => setShowAdd(false)} />

            {editing && (
                <EditAlgorithmModal open={!!editing} onClose={() => setEditing(null)} algorithm={editing} />
            )}
            ...
            {deleting && (
                <DeleteConfirmModal
                    ...
                    title="Deactivate Algorithm?"
                    description={`"${deleting.name}" will be marked inactive. Existing patient assignments are unaffected.`}
                    onConfirm={async () => { await algorithmsApi.delete(deleting.id); await refresh(); }}
                />
            )}

            {applying && (
                <ApplyAlgorithmModal open={!!applying} onClose={() => setApplying(null)} algorithm={applying} />
            )}
        </>
    );
}
