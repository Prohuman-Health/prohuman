...
import {
    ...
    Form, Question, QuestionAnswerType,
} from "@/lib/api";
...
function BuilderModal({ form, onClose, onSaved }: { form: Form | null; onClose: () => void; onSaved: () => void }) {
    ...
    function addFromDirectory(q: Question) {
        setQuestions(prev => [...prev, {
            _key: `dir-${q.id}-${Date.now()}`,
            question_id: q.id,
            text: q.text,
            answer_type: q.answer_type,
            options: q.options ?? [],
    ...
    return (
                        ...
                        <div className="px-6 pb-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-violet-200 bg-violet-50 text-xs font-medium text-violet-700 hover:bg-violet-100 transition-all">
                                    <BookOpen className="w-3.5 h-3.5" />Browse Directory
                                </button>
                            </div>
                        </div>
        ...
        {dirPickerOpen && (
            <DirectoryPicker
                alreadyAdded={questions.map(q => q.question_id).filter(Boolean)}
                onAdd={q => { addFromDirectory(q); }}
...
function DirectoryPicker({
    ...
    onAdd,
    onClose,
}: { alreadyAdded: string[]; onAdd: (q: Question) => void; onClose: () => void }) {
    const [allQ, setAllQ] = useState<Question[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState("");

    useEffect(() => {
        questionsApi.list().then(q => setAllQ(q.filter(q => q.is_active))).catch(() => setAllQ([])).finally(() => setLoading(false));
    }, []);
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-border/60">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-sm">Browse Question Directory</h3>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-xl bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground">
                        <X className="w-4 h-4" />
                    </button>
                ...
                <div className="px-5 py-3 border-b border-border/40">
                    <div className="relative">
                        ...
                        <Input placeholder="Search questions…" className="pl-9 rounded-xl" value={search} onChange={e => setSearch(e.target.value)} />
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
                    {loading ? (
                        Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-12 animate-pulse bg-muted rounded-xl" />)
                    ) : filtered.length === 0 ? (
                        <p className="text-center text-sm text-muted-foreground py-8">
                            {search ? "No matching questions" : "No questions in the directory yet"}
                        </p>
                    ) : filtered.map(q => {
                        const meta = ANSWER_TYPE_META[q.answer_type];
                        const Icon = meta.icon;
                        const added = alreadyAdded.includes(q.id);
                        return (
                            <div key={q.id} className={cn("flex items-start gap-3 px-3 py-2.5 rounded-xl transition-colors", added ? "bg-muted/40" : "hover:bg-muted/50 cursor-pointer")}
                                onClick={() => { if (!added) { onAdd(q); } }}>
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        {q.tags.length > 0 && (
                                            <span className="text-[10px] text-muted-foreground">{q.tags[0]}</span>
                                        )}
                                    </div>
                                </div>
                                {added ? (
                                    <Badge variant="outline" className="text-[10px] rounded-full shrink-0 text-muted-foreground">Added</Badge>
                                ) : (
                                    <button className="w-7 h-7 rounded-lg bg-violet-100 text-violet-700 flex items-center justify-center shrink-0 hover:bg-violet-200 transition-colors">
                ...
                <div className="px-5 py-3 border-t border-border/60 text-xs text-muted-foreground text-center">
                    Click a question to add it to the form · <span className="text-violet-600 font-medium cursor-pointer hover:underline" onClick={onClose}>Done</span>
                </div>
            </div>
        </div>
    );
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-bold">Delete &ldquo;{deleting.title}&rdquo;?</p>
