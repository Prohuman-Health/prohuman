...
function LoginInner() {
    ...
    useEffect(() => {
        ...
        if (err === "access_denied") setError("Your account does not have access to the Front Desk portal.");
    }, [params]);

    // Post-login: if already authenticated, redirect appropriately
    useEffect(() => {
        if (loading || !user) return;
        if (ALLOWED_ROLES.includes(user.role)) {
            router.replace("/dashboard");
        } else {
            setError(`Your role (${user.role}) cannot access the Front Desk portal.`);
        }
    }, [user, loading, router]);
                ...
                <div className="relative z-10 space-y-6">
                    <div className="space-y-3">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#2493A2]/15 border border-[#2493A2]/30 mb-2">
                            ...
                            <span className="text-[#57BDA2] text-xs font-semibold tracking-wide uppercase">Front Desk Portal</span>
                        </div>
                        <h1 className="text-4xl font-bold text-white leading-tight">
                            Scheduling,<br />
                            <span className="text-[#2493A2]">Simplified.</span>
                        </h1>
                        <p className="text-[#57BDA2] text-lg leading-relaxed">
                            Manage appointments, patients, and the daily clinic flow — all from one place.
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {["Session Booking", "Patient Check-In", "Calendar View", "Appointment Requests", "Walk-In Management"].map(f => (
            ...
            <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
                <div className="w-full max-w-md space-y-8">
                    <div className="lg:hidden">
                        <Image src="/logo.png" alt="ProHuman Health" width={180} height={55}
                            className="object-contain" priority
                            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-white/40 mt-1 text-sm">Sign in to your front desk account</p>
                    </div>
                    ...
                    <form onSubmit={handleEmailLogin} className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="you@prohuman.com"
                                className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10
                           text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-[#2493A2] transition-all duration-200" />
                        </div>
                    ...
                    <p className="text-center text-white/20 text-xs">
                        Only authorised front desk staff can access this portal.
                    </p>
                </div>
            </div>
        </div>
