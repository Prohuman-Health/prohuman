...
function ChannelItem({ channel, active, onClick }: {
    channel: StaffChannel; active: boolean; onClick: () => void;
}) {
        ...
        <div className={cn("flex gap-2 max-w-[75%]", isOwn ? "ml-auto flex-row-reverse" : "")}>
            ...
            <div>
                ...
                <div className={cn(
                    ...
                    isOwn ? "bg-[#2493A2] text-white rounded-tr-sm" : "bg-white border border-border/60 rounded-tl-sm"
                )}>
                    {msg.body}
                </div>
        ...
        <div className="flex h-[calc(100vh-56px)] overflow-hidden">
            <div className="w-72 border-r border-border/60 bg-white flex flex-col shrink-0">
                <div className="p-4 border-b border-border/60">
                    ...
                    <div className="relative">
                        ...
                        <Input placeholder="Search channels…" className="pl-7 h-8 text-xs rounded-xl"
                            value={channelSearch} onChange={e => setChannelSearch(e.target.value)} />
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
                    {loadingChannels && [...Array(5)].map((_, i) => (
                        <div key={i} className="h-12 rounded-xl bg-muted animate-pulse mx-1" />
                    ))}
                ...
                <div className="flex-1 flex flex-col overflow-hidden bg-muted/20">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-3">
                        ...
                        {messages.map((msg, idx) => {
                            ...
                            return <MessageBubble key={msg.id} msg={msg} isOwn={msg.sender_id === myId} showSender={showSender} />;
                        })}
                    ...
                    <div className="px-5 py-3 border-t border-border/60 bg-white shrink-0">
                        <div className="flex gap-2 items-end">
                            <Input placeholder={`Message ${displayName ?? "channel"}…`}
                                className="flex-1 rounded-xl text-sm"
                                value={draft}
                                onChange={e => setDraft(e.target.value)}
                                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } }}
                            />
                            <Button className="rounded-xl h-10 w-10 p-0 bg-[#2493A2] hover:bg-[#1e7d8c] shrink-0"
                                onClick={sendMessage} disabled={!draft.trim() || sending}>
            ...
            {showNewDM && (
                <NewDMDialog
                    ...
                    onDM={id => { setShowNewDM(false); loadChannels(); setActiveChannelId(id); }}
                />
            )}
        </div>
    );
