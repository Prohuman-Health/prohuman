...
import {
    ...
    CalendarDays, Activity, Plus, ChevronLeft, ChevronRight,
    AlertCircle, Loader2,
} from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
    ...
    "late-cancellation": "bg-orange-100 text-orange-600",
};
...
export default function DoctorDetailPage() {
    ...
    useEffect(() => {
        if (doctor && (tab === "sessions" || tab === "calendar")) loadSessions(sessionsPage);
        ...
        <div className="flex flex-col h-full overflow-hidden gap-4 p-4 md:p-5">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <h1 className="text-xl font-bold tracking-tight truncate">{doctor.full_name}</h1>
                    {doctor.specialty && <p className="text-xs text-muted-foreground">{doctor.specialty}</p>}
                </div>
                <Button size="sm" className="rounded-xl gap-1.5 text-xs shrink-0" onClick={() => setScheduleOpen(true)}>
            ...
            <div className="flex gap-4 flex-1 min-h-0 flex-col lg:flex-row">
                ...
                {/* Left sidebar */}
                <div className="lg:w-64 shrink-0 flex flex-row lg:flex-col gap-3 overflow-x-auto lg:overflow-y-auto lg:overflow-x-visible">
                    {/* Profile card */}
                    <div className="bg-white rounded-2xl p-4 border border-border/50 flex flex-col items-center gap-3 text-center shrink-0 lg:shrink w-56 lg:w-auto">
                        <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold", AVATAR_COLORS[0])}>
                            {initials}
                        </div>
                        <div>
                        ...
                        <Badge variant="outline" className={cn("text-[10px] rounded-full px-2.5 font-medium",
                            doctor.is_active ? "border-emerald-200 text-emerald-700 bg-emerald-50" : "border-red-200 text-red-600 bg-red-50")}>
                            {doctor.is_active ? "Active" : "Unavailable"}
                        </Badge>
                    </div>

                    {/* Contact card */}
                    <div className="bg-white rounded-2xl p-4 border border-border/50 space-y-3 shrink-0 lg:shrink w-56 lg:w-auto">
                    ...
                    {/* Bio card */}
                    {doctor.bio && (
                        <div className="bg-white rounded-2xl p-4 border border-border/50 shrink-0 lg:shrink w-56 lg:w-auto">
                    ...
                    {/* Quick stats */}
                    <div className="bg-white rounded-2xl p-4 border border-border/50 grid grid-cols-2 gap-3 shrink-0 lg:shrink w-56 lg:w-auto">
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden">
                                            ...
                                            return (
                                                <tr key={s.id} className="border-b border-border/60 hover:bg-muted/20 transition-colors">
                                                    ...
                                                    <td className="px-5 py-3.5 whitespace-nowrap">
                                                        <p className="text-xs font-medium">{dt.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
                                                        <p className="text-[11px] text-muted-foreground">{dt.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true })}</p>
                                                    </td>
                                                    <td className="px-5 py-3.5 text-xs text-muted-foreground">{s.session_type_name}</td>
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-between px-5 py-3 border-t border-border/60 text-xs text-muted-foreground">
                                    <span>{(sessionsPage - 1) * SESSIONS_LIMIT + 1}–{Math.min(sessionsPage * SESSIONS_LIMIT, sessionsTotal)} of {sessionsTotal}</span>
                                ...
                                {DAYS.map((dayName, dayIdx) => {
                                    ...
                                    return (
                                        <div key={dayIdx} className="bg-muted/30 rounded-xl p-3.5">
                                            <p className="text-xs font-semibold mb-2.5">{dayName}</p>
                                            {daySlots.length === 0 ? (
                                                ...
                                                <div className="space-y-1.5">
                                                    {daySlots.map(slot => (
                                                        <div key={slot.id} className={cn(
                                                            ...
                                                            {slot.label && (
                                                                <span className="text-muted-foreground text-[10px]">{slot.label}</span>
                                                            )}
                                                        </div>
                                                    ))}
                                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto">
                                ...
                                <div className="px-5 pt-4 pb-5">
                                    ...
                                    <button onClick={() => setScheduleOpen(true)}
                                        ...
                                        <Plus className="w-3 h-3" /> Schedule session on this day
                                    </button>
                                </div>
                            </div>
                        </div>
